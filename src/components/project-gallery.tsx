    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        {images.map((image) => (
          ...
          >
            ...
            >
              {/* Native img preserves PNG sharpness (no WebP/AVIF recompression) */}
              <img
                src={image.src}
                alt={image.alt}
                width={image.width}
                height={image.height}
                className="mx-auto h-auto w-full max-w-[576px]"
                decoding="sync"
                loading="eager"
              />
            </a>
      ...
      {projectTitle ? (
        <p className="text-center text-xs text-muted">
          {projectTitle} — powered by SBJ Studio. Files are served at original quality from this site.
        </p>
      ) : null}
    </div>
  );
